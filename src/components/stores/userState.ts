...
import { supabase } from '@c/supabase/supabaseClient';
...
export async function syncSupabaseUser() {
  try {
    // Check if supabase is not null
    if (!supabase) {
      console.error('[syncSupabaseUser] Supabase client is null');
      clearUserState();
      return;
    }
...
export async function syncUserMemeProfile(identifier: string, useCache = true) {
  ...
  try {
    // Check if supabase is not null
    if (!supabase) {
      console.error('[syncUserMemeProfile] Supabase client is null');
      userMemeProfileAtom.set(undefined);
      return;
    }
...
export async function logoutUser() {
  try {
    // Sign out from Supabase if supabase is not null
    if (supabase) {
      ...
      console.error('[logoutUser] Supabase client is null');
    }
...
export async function createUserProfile(username: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    // Check if supabase is not null
    if (!supabase) {
      console.error('[createUserProfile] Supabase client is null');
...
export async function getUsernameByUuid(uuid: string): Promise<string | null> {
  try {
    if (!supabase) {
      console.error('[getUsernameByUuid] Supabase client is null');
      return null;
    }
...
export async function getUuidByUsername(username: string): Promise<string | null> {
  try {
    if (!supabase) {
      console.error('[getUuidByUsername] Supabase client is null');
      return null;
    }
...
export function initializeAuthListener() {
  ...
  if (!supabase) {
    console.error('[initializeAuthListener] Supabase client is null');
    return;
  }
